  ...
  setup() {
      ...
      onSnapshot(
        ...
        (error) => {
          ...
          if (error.code === "permission-denied") {
            ...
            showAlert("無權限存取資料，請檢查 Firestore 規則設定", "error");
          }
          loading.value = false;
        },
      );
      ...
      if (sortableList.value) {
        ...
        sortableInstance = new Sortable(sortableList.value, {
          ...
          onEnd: async (evt) => {
            ...
            const newOrderIds = Array.from(sortableList.value.children).map(
    ...
    const submitForm = async () => {
      ...
      if (
        ...
        const duplicateExtra = findDuplicateAddressInExtra(form.value.address);
        if (duplicateExtra) {
          const { ElMessageBox } = window.ElementPlus;
          try {
      ...
      try {
        ...
        const collectionName = isExtra ? "travel_extra" : "travel_schedule";
        const payload = {
        ...
        if (isEditing.value && form.value.id) {
          await updateDoc(doc(db, collectionName, form.value.id), payload);
        } else {
    ...
    const getCategoryColor = (tag) => categoryMap[tag]?.color || "bg-gray-400";
